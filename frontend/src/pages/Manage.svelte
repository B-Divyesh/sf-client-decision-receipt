...
<section class="narrow manage-page">
  ...
  {:else if record}
    ...
    {#if record.decision}<section class="delivery"><h2>Receipt delivery</h2><p>Each supplied address receives a receipt when mail delivery is configured.</p>{#if record.delivery.length}<ul>{#each record.delivery as delivery}<li><span>{delivery.recipient}</span><strong>{delivery.status === 'sender_not_configured' ? 'sender not configured' : delivery.status}</strong></li>{/each}</ul>{/if}</section>{/if}
